...
export default class ReactNativeJellySideMenu extends Component {
  constructor(props) {
    super(props);
    this.state = {
      is_dock: false,
  ...
  onDragSideMenuSvg(x, y) {
    this.refs.sideMenuSvgWrapper
      ? this.refs.sideMenuSvgWrapper.onJellyNotUndocked()
      : {};
    this.refs.sideMenuSvg
      ? this.refs.sideMenuSvg.setOffsetDrag(x, y, false)
      : {};
  }
  ...
  onJellyUndocked() {
    this.refs.sideMenuSvgWrapper
      ? this.refs.sideMenuSvgWrapper.onJellyUndocked()
      : {};
  }
          ...
          style={[
            {
              ...
              left: 0,
              bottom: 0,
              backgroundColor: this.state.is_dock
                ? "rgba(0,0,0,0.5)"
                : "rgba(0,0,0,0)",
            },
            dockStyle,
      ...
      return (
        ...
        >
          <JellySideMenuSvg
            ...
            fill={this.props.fill || "#FFF"}
            fillOpacity={this.props.fillOpacity || 0.9}
            height={height}
            dockWidth={dockWidth}
            ref={"sideMenuSvg"}
          />
...
class JellySideMenuContent extends Component {
  ...
  componentWillUpdate(nextProps, nextState) {
    if (nextProps.is_dock != this.props.is_dock) {
      if (nextProps.is_dock) {
        Animated.spring(this.state.dockOpacityAnim, {
          toValue: 1,
          friction: 10,
        }).start();
        Animated.spring(this.state.dockLeftAnim, {
          toValue: 0,
          friction: 5,
        }).start();
      } else {
  ...
  render() {
    var style = {
      width: this.props.dockWidth,
      position: "absolute",
      top: 0,
      opacity: this.state.dockOpacityAnim,
      left: this.state.dockLeftAnim,
      bottom: 0,
...
class JellySideMenuSvg extends Component {
  constructor(props) {
    ...
    this.ssOffsetDragX.addListener({
      onSpringUpdate: () => {
        if (!this.is_mounted) {
          return;
        }
        ...
        if (this.ssOffsetDragX.getEndValue() <= 0) {
          if (this.state.offsetDragX <= 0 && !this.state.is_undocked) {
            this.setState({
              offsetDragX: this.ssOffsetDragX.getCurrentValue(),
              is_undocked: true,
            });
            this.ssOffsetDragX.setCurrentValue(0);
    ...
    this.ssOffsetDragY.addListener({
      onSpringUpdate: () => {
        if (!this.is_mounted) {
          return;
        }
    ...
    this.ssOffsetDragXSm.setCurrentValue(0);
    this.ssOffsetDragXSm.addListener({
      onSpringUpdate: () => {
        if (!this.is_mounted) {
          return;
        }
        ...
        this.isBusySm = true;
        this.setState({
          offsetDragXSm: this.ssOffsetDragXSm.getCurrentValue(),
        });
      },
    });
  ...
  render() {
    ...
    pathSide +=
      " Q" + offsetDragX + " " + 0 + " " + offsetDragX + " " + offsetDragY;
    pathSide +=
      " Q" + offsetDragX + " " + height + " " + offsetDragXSm + " " + height;
