...
import { Animated, Image, View, Platform } from "react-native";
import TransformUtil from "./TransformUtil";
import {
  FOLDING_ANIMATION_PART_KIND,
  FoldingAnimationPartKindType,
} from "./types";
...
function PaperFoldingAnimationPart({
  ...
  const transformFolding = useCallback(
    (deg: number) => {
      let matrix;

      if (kind === FOLDING_ANIMATION_PART_KIND.TOP_TO_BOTTOM) {
        matrix = TransformUtil.rotateX(
          -1 * deg,
          transformBasePosition,
          PERSPECTIVE
        );
