...
import { View, ViewPropTypes, Text, Platform } from "react-native";
...
const { OS } = Platform;
const DEFAULT_COLOR_BUTTON = colors.violet600;
...
class ConfirmDialog extends Component {
  ...
  renderMessage() {
    ...
    if (message)
      return (
        <Text
          style={[
            { textAlign, color: colors.coolGray600, fontSize: 18 },
            messageStyle,
          ]}
        >
          {message}
  ...
  renderButtons() {
    ...
    const dividerVertStyle =
      OS === "ios"
        ? { width: negativeButton ? 1 : 0, backgroundColor: colors.coolGray300 }
        : { width: 8 };

    const dividerHoriStyle =
      OS === "ios"
        ? { height: 1, backgroundColor: colors.coolGray300 }
        : { height: 0 };
  ...
  renderContent() {
    ...
    else return this.renderMessage();
  }
