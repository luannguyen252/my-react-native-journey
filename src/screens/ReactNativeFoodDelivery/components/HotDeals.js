...
class HotDeals extends Component {
  ...
  renderEntity(entity: entity, sectionID: number, rowID: number) {
    return <DishItem entity={entity} addToCart={_addToCart} />;
  }
...
class DishItem extends Component {
    ...
    return (
      <View style={styles.item}>
        <View style={styles.item_top_info}>
          <Text style={styles.classic}>CLASSIC</Text>
        </View>
        ...
        <View style={styles.item_bottom_info}>
          <View style={styles.details_container}>
            <Text style={styles.details}>
              Rice, Extra Chicken, Extra Paneer, Extra Cheese
            </Text>
          </View>
          <View style={styles.counter_container}>
            ...
            <View style={styles.buttons}>
              {this.state.quantity > 0 && (
                <TouchableHighlight
                  style={{ paddingLeft: 8 }}
                  onPress={() => this.removeDish(entity)}
                  underlayColor="white"
                >
                  <Icon
  ...
  removeDish(entity) {
    this.setState({
      quantity:
        this.state.quantity > 0 ? this.state.quantity - 1 : this.state.quantity,
    });
  }
}
