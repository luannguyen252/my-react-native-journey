...
const Topics = () => {
  ...
  const handleItemLayout = (event, topic) => {
    if (!topicPositionRef.current[topic]) {
      ...
      if (Object.keys(topicPositionRef.current).length === topics.length) {
        const topicPosition = Object.values(topicPositionRef.current).reduce(
          (accumulator, current) => {
  ...
  const Topic = ({ topic }) => {
    ...
    const style = useAnimatedStyle(() => {
      ...
      if (levelWidth) {
        const levelDifference =
          screenWidth - levelWidth[topicPosition[text].layout.y];
        const maxLevelDifference =
    ...
    return (
      ...
      >
        <Text>{emoji}</Text>
        <Text style={topicText}>{text}</Text>
      </Animated.View>
    );
  };
  ...
  return (
    <PanGestureHandler {...{ onGestureEvent }}>
      <Animated.View>
        <Text style={title}>{"TOPICS TO EXPLORE"}</Text>
        <Animated.View horizontal style={container}>
          {topics.map((topic) => (
