...
export default class GrippableView extends Component {
  ...
  constructor(props: Props) {
    ...
    this._panResponder = PanResponder.create({
      ...
      onPanResponderMove: (evt, gestureState) =>
        this._moveSubject.next(gestureState.dy),
    ...
    const [forward, backward] = source
      ...
      .partition(
        (values) =>
          last(values) >= this.props.initExpandHeight &&
          last(values) > values[values.length - 2]
      );

    this._moveSubscription = source.subscribe((x) =>
      this.state.animatedHeight.setValue(x)
    );

    this._forwardSubscription = forward.subscribe(() =>
      this.spring(this.props.maxHeight)
    );

    this._backwardSubscription = backward.subscribe(() =>
      this.spring(this.props.minHeight)
    );
  }
  ...
  spring(toValue: number) {
    ...
    }).start(() => this.setState({ height: toValue }));
  }
  ...
  render() {
    ...
    return (
      ...
      >
        {children}
        <View style={gripperStyles}>{this.props.getGripperComponent()}</View>
      </Animated.View>
    );
  }
}
