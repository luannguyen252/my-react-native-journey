...
const BubbleBox = ({
  ...
  const { boxModes, setBoxFull, setCurrentCardId } =
    useContext(BubbleBoxContext);

  const boxMode = boxModes[index];

  const { effectPath, changeEffectPath } = useBubbleBoxEffect();
  ...
  useEffect(() => {
    ...
    if (
      boxMode === BUBBLE_BOX_MODE.HIDDEN &&
      currentMode === BUBBLE_BOX_MODE.NORMAL
    ) {
