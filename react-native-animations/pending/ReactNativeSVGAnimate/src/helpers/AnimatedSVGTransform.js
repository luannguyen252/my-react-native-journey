...
function universal2axis(key, nextProps, prevProps, defaultValue) {
  let [nextX, nextY] = _universal2axis(
    nextProps[key],
    nextProps[key + "X"],
    nextProps[key + "Y"]
  );
  let [prevX, prevY] = _universal2axis(
    prevProps[key],
    prevProps[key + "X"],
    prevProps[key + "Y"],
    defaultValue
...
function createTransformObject(nextProps, prevProps) {
  ...
  const [scaleX, scaleY] = universal2axis("scale", nextProps, prevProps, 5);
    ...
    setNativeProps = (props) => {
      ...
      if (KEYS.some((key, index) => props[key] != null)) {
        const matrix = createTransformMatrix(
          createTransformObject(props, this.prevProps)
        );
        let x, y;
