...
class App extends React.Component {
  ...
  getMoviePosterByQuote(quote) {
    return fetch(
      `${TMDB_SEARCH_URI}?api_key=${Config.TMDB_KEY}&query=${quote.author}`
    )
      .then((x) => x.json())
      .then((y) => {
  ...
  render() {
    const {
      info: { quote, poster, oldPoster, author },
      isLoading,
      currentBgColor,
      nextBgColor,
    } = this.state;
    ...
    return (
      ...
      >
        <View style={styles.container}>
          ...
          <Animated.View
            style={[
              styles.backgroundImageContainerBackdrop,
              { backgroundColor: bg },
            ]}
          ></Animated.View>
        </View>
      ...
      >
        <Animated.Image
          ...
          style={[
            ...
            styles.full,
            { opacity: this.state.opacity, position: "absolute", zIndex: 2 },
          ]}
          onLoad={() => {
        ...
        <Animated.Image
          ...
          style={[
            styles.backgroundImage,
            styles.full,
            { opacity: this.state.revertOpacity },
          ]}
        />
      </Animated.View>
    );
