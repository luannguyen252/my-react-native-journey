...
class MorphSlider extends React.Component {
  ...
  _animateTextOpacityDelayed = debounce(
    this._animateTextOpacity.bind(this),
    20
  );
      ...
      >
        <TouchableView
          ...
          onTouchesMoved={({ gestureState }) => {
            if (Math.abs(gestureState.dx) > 0 || this.touchesGranted) {
              //workaround android invoking move
              this.touchesGranted = true;
              this._absoluteTranslateX.setValue(
                this._getOptimizedAbsoluteX(gestureState.moveX)
              );
              this._nonNativeX.setValue(
                this._getOptimizedAbsoluteX(gestureState.moveX)
              );
              this._ballTranslateX.setValue(
                this._getOptimizedX(gestureState.moveX)
              );
            }
          }}
        >
