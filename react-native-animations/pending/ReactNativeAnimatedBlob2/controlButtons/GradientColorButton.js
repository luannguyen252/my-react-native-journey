...
import { Surface, Shape, LinearGradient } from "@react-native-community/art";
...
const GradientColorButton = ({ colors, gradientColors, onPress }: Props) => {
  const layout = useMemo(
    () => ({
      selectStyle: {
        ...styles.select,
        borderColor:
          gradientColors?.start === colors.start &&
          gradientColors?.end === colors.end
            ? colors.end
            : "transparent",
      },
