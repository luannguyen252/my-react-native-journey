...
export default class AnimatedSwiper extends Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    if (
      ...
      (!this.state.swipe && nextState.swipe)
    ) {
      return true;
    }
...
class PageComponent extends Component {
    ...
    return [
      ...
      },
      {
        opacity: this.state.containerTranslateX.interpolate({
          inputRange: [
            -this.props.dragOffsetForTransparency,
            0,
            this.props.dragOffsetForTransparency,
          ],
          outputRange: [0, 1, 0],
        ...
        transform: [
          ...
          },
          {
            rotate: this.state.containerTranslateX.interpolate({
              inputRange: [
                -dragOffsetForTransparency,
                0,
                dragOffsetForTransparency,
              ],
              outputRange:
                this.props.nextPageRotate.length === 3
                  ? this.props.nextPageRotate
                  : ["0deg", "0deg", "0deg"],
            }),
          },
        ],
        ...
        transform: [
          ...
          },
          {
            rotate: this.state.containerTranslateX.interpolate({
              ...
              outputRange:
                this.props.currentPageRotate.length === 3
                  ? this.props.currentPageRotate
                  : ["0deg", "0deg", "0deg"],
            }),
          },
        ],
      ...
      onPanResponderRelease: (evt, gestureState) => {
        if (this.state.containerTranslateX._value < -100) {
          if (this.props.page < this.props.countPage - 1) {
            ...
            Animated.spring(this.state.containerTranslateX, {
              toValue: -dragOffsetForTransparency,
        ...
        if (
          this.state.containerTranslateX._value < 100 &&
          this.state.containerTranslateX._value > -100
        ) {
          Animated.spring(this.state.containerTranslateX, {
            toValue: 0,
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (this.props.page === nextProps.currentPage && nextProps.swipe) {
      ...
      Animated.timing(this.state.containerTranslateX, {
        toValue: 0,
        duration: this.props.nextPageAnimationDuration,
        useNativeDriver: true,
      }).start(() => {
        this.props.changeSwipeState(false);
  ...
  shouldComponentUpdate(nextProps) {
    if (
      nextProps.swipe ||
      Math.abs(this.props.page - nextProps.currentPage) > 1
    )
      return false;
    return true;
  }
    ...
    return (
      <View style={styles.swipeble}>
        <Animated.View
          ...
          style={
            this.props.page === this.props.currentPage
              ? this.animatedStyle()
              : this.nextPageAnimatedStyle()
          }
