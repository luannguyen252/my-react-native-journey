...
export default class ReactNativeAnimatedGallery extends Component {
  constructor(props) {
    super(props);
    ...
    this.initialXPositions = [
      this.state.viewWidth / 2,
      -100,
      0,
      0,
      0,
    ...
    for (let i = 0; i < imagesCount; i++) {
      ...
      this.animatedGridPositions.push(
        new Animated.ValueXY({
          x: this.initialXPositions[i],
          y: this.initialYPositions[i],
        })
      );
      this.animatedImageHeights.push(
        new Animated.Value(
          this.getImgHeight(images[i].url, this.state.viewWidth / 2.4)
        )
      );
      ...
      if (i == 0 || i == 1) {
        ...
        gridYPositions[i + 2] =
          gridYPositions[i] +
          this.getImgHeight(images[i].url, this.state.imageWidth) +
          50;
        ...
        Animated.parallel([
          ...
          Animated.timing(this.animatedGridPositions[i], {
            ...
            toValue: {
              x:
                i % 2
                  ? this.state.viewWidth / (this.widthOffsetRation - 0.2)
                  : 0,
              y: gridYPositions[i],
            },
            duration: 600,
          ...
          Animated.spring(this.animatedImageHeights[i], {
            toValue: this.getImgHeight(
              images[i].url,
              this.state.viewWidth / 2.4
            ),
          }),
    ...
    for (let i = imagesCount - 1, offsetIndex = 0; i >= 0; i--, offsetIndex++) {
      ...
      this.scrollYOffsets.push(
        this.scrollYOffsets[offsetIndex] +
          this.getImgHeight(images[i].url, this.state.viewWidth / 1.5) +
          10
      );
    }
  }
  ...
  clickSideMenu() {
    ...
    Animated.spring(this.state.scrollValue, {
      toValue: this.state.isSideMenuOpen ? 0 : 1,
    }).start();
  }

  moveYearTop(year) {
    this.setState({ isYearSelected: true, yearLabels: [year] }, () => {
      this.setState({ imageWidth: this.state.viewWidth / 1.5 }, () =>
        this.animateImagesToScroll()
      );
        ...
        <View style={styles.mainContentStyle}>
          ...
          <ScrollView
            contentContainerStyle={{
              height: this.state.viewHeight * images.length,
            }}
              ...
              return (
                <Animated.View
                  ...
                  style={{
                    ...
                    transform:
                      this.animatedGridPositions[index].getTranslateTransform(),
                  }}
                >
                  <TouchableHighlight onPress={() => this.openImageMenu(index)}>
                    <Animated.Image
                      source={image.url}
                      style={{
                        marginLeft: this.animatedMarginLeft[index],
                        width: this.animatedImageWidths[index],
                        height: this.animatedImageHeights[index],
                      }}
                    />
                  </TouchableHighlight>
        ...
        >
          <View style={styles.toggleContainerStyle}>
            <TouchableHighlight
              ...
              onPress={this.clickSideMenu.bind(this)}
            >
              <Icon
                name={
              ...
              return (
                <Animated.View
                  ...
                  style={[
                    ...
                    index == 0
                      ? activeYearLabelContainerStyle
                      : styles.yearLabelContainerStyle,
                    this.state.isYearSelected
                      ? { marginTop: this.animatedMarginBottomValue }
                      : null,
                  ]}
                >
